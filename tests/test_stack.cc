...
bool compare_stacks(s21::stack<value_type1> my_stack,
...
TEST(stack, copy_constructor_3) {
  const s21::stack<char> A{'a', 'b', 'c'};
  const s21::stack<char> B(A);
  std::stack<char> C;
  for (char i = 'a'; i < 'd'; ++i) C.push(i);
...
TEST(stack, func_size_2) {
  const s21::stack<int> A{6, 7, 8, 9, 78};
  EXPECT_EQ(5, A.size());
}

TEST(stack, func_push_1) {
  s21::stack<int> A{6, 7, 8, 9, 78};
  A.push(187);
  EXPECT_EQ(187, A.top());
}
...
TEST(stack, func_swap_1) {
  s21::stack<char> A{'a', 'd', 'g'};
  s21::stack<char> B{'c', 'g', 'y'};
  std::stack<char> C;
  C.push('a');
  C.push('d');
  C.push('g');
